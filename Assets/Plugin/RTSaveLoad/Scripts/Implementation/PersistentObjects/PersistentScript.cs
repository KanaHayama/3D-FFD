...
namespace Battlehub.RTSaveLoad
{
    ...
    public class PersistentScript : PersistentObjects.PersistentObject
    {
        ...
        {
            base.ReadFrom(obj);
            fields = new Dictionary<string, DataContract>();
                        ...
                        else
                        {
                            if(!field.FieldType.IsEnum())
                            {
                                if(field.FieldType.IsPrimitive() || field.FieldType.IsArray())
                                {
                                    ...
                                    primitive.ValueBase = field.GetValue(obj);
                                    this.fields.Add(field.Name, new DataContract(primitive));
                                }
                                else
                                {
                                    this.fields.Add(field.Name, new DataContract { Data = field.GetValue(obj) });
                                }
                            }
                            else
                            {
                            ...
                            else
                            {
                                if (!field.FieldType.IsEnum())
                                {
                                    if(field.FieldType.IsPrimitive() || field.FieldType.IsArray())
                                    {
                                        ...
                                        field.SetValue(obj, primitive.ValueBase);
                                    }
                                    else
                                    {
                                        field.SetValue(obj, value.Data);
                                    }
                                }
